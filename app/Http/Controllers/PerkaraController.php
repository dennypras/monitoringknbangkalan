...
class PerkaraController extends Controller
{
    ...
    public function updateKategori(Request $request, $id)
    {
        ...
        switch ($next) {
            case 'Berkas Tahap 1':
                $detail->nomor_berkas_perkara = $validated['nomor_berkas'] ?? null;
                $detail->nomor_pengantar_berkas = $validated['nomor_pengantar']?? null;
                $detail->tanggal_berkas_tahap_1 = $validated['tanggal_berkas']?? null;
                break;
            case 'P-18':
                $detail->tanggal_p18 = $validated['tanggal_p18'] ?? null;
                break;
            case 'P-19':
                $detail->tanggal_p19 = $validated['tanggal_p19'] ?? null;
                break;
            case 'Berkas Tahap 1 Kembali':
                $detail->tanggal_berkas_kembali = $validated['tanggal_berkas_kembali'] ?? null;
                $detail->nomor_pengantar_berkas_kembali = $validated['tanggal_pengantar_berkas_kembali'] ?? null;
                break;
            case 'P-21':
                $detail->tanggal_p21 = $validated['tanggal_p21'] ?? null;
                break;
        }
...
{
    $berkas = Perkara::with('kategori.detailKategori')->findOrFail($id);
